...
- **Roshaan — model-specific branches (2026-06-10, backend on `backend`, frontend on `roshaan/model-branches` off your `jayden/frontend`) — @Jayden please read:**
  ...
  - **Backend (mine, done, 68 tests green, live-verified against real Gemini):**
    - `POST /api/chat/{provider}` is now generic over a provider registry (`app/services/providers/`: base interface + gemini/openai/anthropic/ollama). **Your `/api/chat/gemini` contract is UNCHANGED and verified** — request gains an OPTIONAL `model` field (absent = old behaviour exactly), response gains `provider` + `model` (additive; your current `api.ts` ignores them fine).
    - New: `GET /api/models` (available models + metadata, filtered to configured providers) and `POST /api/models/recommend` (rule-based "recommended for this task" ranking with reason strings). Model ids are validated against a server-side catalog (`app/services/model_catalog.py`); unknown provider = 404, unknown model = 422, unconfigured provider = 503 — all `{detail}` user-facing copy.
    - Errors before quota: provider/model validation happens BEFORE the quota charge, and provider failures refund (the refund-on-5xx you asked about — that question is answered: implemented).
    - Deploy note (me): OPENAI_API_KEY / ANTHROPIC_API_KEY are new OPTIONAL Railway envs; without them those providers simply don't appear in /api/models. Nothing breaks if absent.
  - **Frontend (PUSHED on `roshaan/model-branches`, YOUR files touched — review before merging):** this feature genuinely needed `chatStore.ts`/`api.ts`/`types/chat.ts`/`InputBar.tsx`/`ChatNode.tsx`, so per the high-risk-files rule, it's on its own branch off your latest `jayden/frontend` for you to review/merge, not pushed into your branch. Changes: `ChatNode.modelOverride` + per-node `provider`/`model` stamps, `resolveModelForNode` (nearest-ancestor model resolution), `branchFromNode` opts gain `model`, new `ModelPicker.tsx` dialog (recommendations from the backend, manual pick below; redesigned after a design pass — commit `d23ce06`), "Branch with model" in the composer + node hover action, model badge on AI replies, composer line shows the active branch model. `requestChatReply` signature: returns `{reply, provider, model}` and takes `{provider}` opts — additive otherwise. 4 new store tests; 32 frontend tests green, build green, lint = your 3 pre-existing known errors only. Verified live in Chrome end-to-end (pick → branch → real Gemini reply with model badge). Full details + your action list in the todo.md on THAT branch.
  - Persisted shape: only OPTIONAL node fields added — old localStorage saves load unchanged (no persist version bump needed).
  - Docs updated for the feature: `architecture.md` (AI flow, routers, services, env — on both branches), backend `CLAUDE.md` file map, `.env.backend.example`, frontend `.env.example` + `CLAUDE.md`. `README_BACKEND.md` now carries a prominent stale-doc banner pointing at the current docs (the P2 "remove stale OpenAI-default wording" item — superseded by the banner until someone rewrites it).
...
- [x] ~~Update `README_BACKEND.md` to remove stale OpenAI-default wording~~ — superseded 2026-06-10: the doc now opens with a stale-doc banner pointing at `architecture.md`/`CLAUDE.md` (the chat path is now provider-generic, not Gemini-first, so a full rewrite is a separate task if we ever want this doc back).
