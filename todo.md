...
- [x] **Dark mode (Jayden, 2026-06-11).** The shadcn token set already shipped a full `.dark` palette + `@custom-variant dark` — nothing ever applied the class. Added `src/lib/theme.ts` (light/dark, defaults to the OS `prefers-color-scheme`, stores the explicit choice under `branchchat-theme`, `useTheme()` via `useSyncExternalStore`, cross-tab `storage` sync), a `ThemeToggle` Sun/Moon icon button mounted in both the `/app` header and the landing header, and an inline boot script in `index.html` that applies the saved/system theme before first paint (no white flash) and keeps `<meta name=theme-color>` in step (#fff / #181818). +4 unit tests (91 total), tsc/lint/build green, verified in headless Chrome (toggle flips `.dark` + bg + meta color, persists across reload, toggles back). **@Roshaan:** purely additive — only touched files are new (`theme.ts`, `ThemeToggle.tsx`, `theme.test.ts`) plus a toggle import/mount in `AppChat.tsx` + `Landing.tsx` and the `index.html` boot script; no conflict with your canvas/markdown batches.
...
- [x] **2026-06-11 canvas batch 2 (Roshaan).** **@Jayden — `Canvas.tsx`, `ChatNode.tsx`, `NodeAnnotations.tsx`, `chatStore.ts`, `treeLayout.ts`, `e2e/smoke.spec.ts` touched again (additive except node-footer layout).** What shipped:
  ...
  - **Sync: always-on + bulletproof deletes** (follow-up commits, frontend + backend 0010). Deletes are now soft on the server: the row becomes a tombstone (payload/title cleared), the manifest broadcasts `deleted: true` so every device drops its copy, a stale offline re-push gets `status: "deleted"` back (client drops its copy), and only a strictly NEWER push resurrects (LWW) — the deleting device then retires its local tombstone and pulls instead of ping-ponging. Local tombstones are now versioned (`{ts, v}`, legacy number shape migrates). **Sync is ALWAYS ON for signed-in users — the SyncToggle is gone** (replaced by a read-only `SyncStatus` row; Roshaan's call: no off switch to lose chats to). Copy updated where it claimed chats never leave the browser (AuthDialog signin, /beta footer, Privacy policy). Files: `sync.ts`, `syncTombstones.ts`, `chatStore.ts`, `Toolbar.tsx`, `api.ts`, backend `routers/sync.py`. 101 frontend + 111 backend tests.
  ...
  - **Beta entry flow** (follow-up commit, pre-launch-email cleanup): the landing footer's "Developer access →" is now "Sign in →" (`/app?auth=signin`); /beta has the primary "Create your account" (`/app?auth=signup`) plus an "Already have an account? Sign in" line; AppGate reads `?auth=` and auto-opens AuthDialog on that form (new `initialMode` prop); ResetPassword's "Continue to sign in" deep-links too. A signed-in approved tester hitting these URLs goes straight to the app. +2 gate tests.
