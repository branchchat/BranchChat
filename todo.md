...
- **Homepage SEO pass (2026-06-10) — branch `roshaan/landing` (Roshaan) — @Jayden FYI, no action needed:**
  ...
  - Follow-ups (not done): per-route `document.title` for `/privacy`, `/terms`, `/app` (they share the homepage title — @Jayden this touches your routing area, a tiny effect or react-helmet would do); `www.branch-chat.com` has NO DNS record (add a CNAME + redirect to apex, Cloudflare-side, human task); main JS bundle is 861 kB (278 kB gzip) — code-splitting would help LCP/INP; after deploy, submit the sitemap in Google Search Console (human task).
