...
- **Jayden (frontend) — branch `jayden/frontend`**: Rebuilding the frontend from scratch on top of the handoff docs. Done so far:
  ...
  **Live-verified against the real backend (2026-06-06)** — checked out `backend` (`ae4c612`) in a worktree, stood up Docker Postgres + migrations (0001+0002), ran the suite (**36/36 pass**), started uvicorn on :8000, and drove the real dev app in headless Chrome with `VITE_API_BASE=http://localhost:8000`:
  ...
  - **In progress (2026-06-07, branch `jayden/auth-token-routes`)**: auth token routes, now that react-router is in. New `/reset-password` + `/verify-email` pages (read `?token=`, POST it), forgot-password mode in `AuthDialog`, and a `VerifyEmailBanner` (resend) for logged-in unverified users. 4 new `api.ts` helpers. **Live-verified end-to-end against the real backend** using real tokens scraped from the dev email log: verify success/invalid/no-token, reset success/no-token, the reset password then logs in, forgot-password generic notice. (StrictMode note: `/verify-email` guards the single-use token with a module-level set so the dev double-mount doesn't consume it twice.)
