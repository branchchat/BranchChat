...
- **Homepage SEO pass (2026-06-10) — branch `roshaan/landing` (Roshaan) — @Jayden FYI, no action needed:**
  ...
  - **Google Search Console — DONE (2026-06-10).** Verified ownership of the `https://branch-chat.com/` URL-prefix property via the **HTML-tag method** (meta `google-site-verification` in `index.html` head, commit `3b55f03`). NOTE: the HTML-**file** method does NOT work on Cloudflare Pages — Pages 308-redirects `*.html` URLs (clean-URL behavior) so `/google<token>.html` is unreachable at its exact path; the meta tag is the reliable method here. **Leave that meta tag in `index.html`** or GSC drops verification. Sitemap `sitemap.xml` submitted and accepted (3 pages discovered). Account: Branchchat@gmail.com.
  - Follow-ups (not done): per-route `document.title` for `/privacy`, `/terms`, `/app` (they share the homepage title — @Jayden this touches your routing area, a tiny effect or react-helmet would do); main JS bundle is 861 kB (278 kB gzip) — code-splitting would help LCP/INP.
  - **HUMAN TODO (couldn't be automated 2026-06-10):**
    1. **`www.branch-chat.com` has NO DNS record.** Add in Cloudflare DNS: CNAME `www` → `branchchat.pages.dev` (proxied), then a Redirect Rule / Bulk Redirect 301 `www.branch-chat.com/*` → `https://branch-chat.com/$1`. Attempted via Chrome but the Cloudflare dashboard SPA would not render in the automation session, and the direct API write was (correctly) blocked by the safety classifier.
    2. **Bing Webmaster Tools** (feeds Microsoft Copilot citations): sign in at bing.com/webmasters, choose "Import from Google Search Console" (fastest — GSC is now verified), authorize, and the verified property + sitemap import in one click. Not done because it requires authenticating/creating an account, which the agent doesn't do on the user's behalf.
