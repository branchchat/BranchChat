...
- [x] `/beta` recruitment page — a focused beta-signup surface (separate from the marketing landing) reusing the **existing waitlist pipe with `source="beta"`** (backend already stores `source`, so **no backend change**): signups land in the same Supabase `waitlist` table, tagged beta, queryable for invites. Matches the landing design (pill/headline/"what to expect" list), `usePageMeta` title, lazy-loaded route, and a "Beta access" link in the landing header. Parametrized the shared `WaitlistForm` (`cta`/`note` props). +1 test. **@Roshaan:** when convenient, add `/beta` to `public/sitemap.xml` on `roshaan/landing` (it's a public recruitment page, should be indexed; it's *not* in robots' disallow list, so crawlable by default).
